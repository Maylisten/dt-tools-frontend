...
const getModelUrl = async (modelName: string, entry: FileSystemEntry) => {
  ...
  const noMainFileTip = `没有找到 .${extension} 后缀的文件！`;
  if (entry.isFile) {
    if (entry.name.endsWith(MODEL_MAIN_FILE_EXTENSION[modelType.value])) {
    ...
    const mainEntry = subEntryList.find(entry => entry.isFile && entry.name.endsWith(extension));
    if (!mainEntry) {
      throw new Error(noMainFileTip);
    }
...
const handleModelUpload = async (entry: FileSystemEntry) => {
  ...
  try {
    ...
    url.value = await getModelUrl(randomModelName, entry);
